...
def CVs_replicas(folder, N_replica, path_ana = False ):
    ...
    if N_replica == 1:
        Cv_N = read_CVs(folder)
        
        Cv_N["Time (ns)"] = Cv_N["Time (ps)"]/1000
        Cv_N["Replica"] = [1 for x in range(len(Cv_N))]
...
def get_mean_hist(data, feature, bindef, frames=None):    
    ...
    hist_rep = []
    for i in np.unique(data["Replica"]):
        if frames:
            hist, y  = np.histogram(data[(data["Replica"] == i) & (data["Time (ns)"] >= frames)][feature],bins = bindef, density = True)
        else:
...
def Write_Probability_Data_Frame(CVs, CV, bin_range, plot_t="bar",figsize=(2.2,1.8), figure_path=None):
    ...
    SymL = get_mean_hist(CVs[CVs["Simulation"] == "NMR, OSymL"], CV, bin_range)
    NOD = get_mean_hist(CVs[CVs["Simulation"] == "NMR, NOD"], CV, bin_range)
    
    df = pd.DataFrame({"Simulation":\
                  ["Unbiased, OSymL" for x in range(len(ub))]*(len(bin_range)-1)+\
                                       ["NMR, OSymL" for x in range(len(SymL))]*(len(bin_range)-1)+\
                                       ["NMR, NOD" for x in range(len(NOD))]*(len(bin_range)-1), "Probability":\
                  [x for y in ub for x in y]+\
                                       [x for y in SymL for x in y]+\
                       [x for y in NOD for x in y],\
    ...
    if plot_t=="line":
        ...
        sns.lineplot(data = df, y="Probability", x=CV, hue = "Simulation",\
                hue_order=["NMR, NOD","Unbiased, OSymL"], ci=68, \
                palette=["orange", "cyan"], markers="o")
        plt.xlim(-bin_range[-1]/11,bin_range[-1])
        #plt.xticks(np.arange(0,len(bin_range),5),np.arange(0,len(bin_range),5))
        if (CV=="Specific Contacts") :
            ...
            plt.xticks(np.arange(0,bin_range[-1],1),np.arange(0,bin_range[-1],1))

        
    #sns.lineplot(data = df, y="Probability", x="Specific Contacts", hue = "Simulation",\
    #            hue_order=["NMR, NOD","Unbiased, OSymL"], ci=68, \
    #            palette=["orange", "cyan"], markers="o")
    ...
    plt.tight_layout()
    plt.savefig(\
    "{}_NOD_last50ns_ub.pdf".format(CV), dpi=300)
    plt.show()
    ...
    if plot_t=="bar":
        sns.barplot(data = df, y="Probability", x=CV, hue = "Simulation",\
                hue_order=["NMR, OSymL","Unbiased, OSymL"], ci=68, errwidth=1, capsize=.15, \
                palette=["purple", "cyan"], errcolor=".5", edgecolor=".5")
        plt.xlim(-1,bin_range[-1])
        plt.ylim(-0.1,1.1)
    ...
    if plot_t=="line":
        ...
        df[CV]=[x for y in [bin_range[:-1] for x in range(11)] for x in y]
        sns.lineplot(data = df, y="Probability", x=CV, hue = "Simulation",\
                hue_order=["NMR, OSymL","Unbiased, OSymL"], ci=68, \
                palette=["purple", "cyan"], markers="o")
        plt.xlim(-bin_range[-1]/11,bin_range[-1])
        plt.xticks(np.arange(0,bin_range[-1],5),np.arange(0,bin_range[-1],5))
        if (CV=="Specific Contacts") :
            plt.xticks(np.arange(0,bin_range[-1],5),np.arange(0,bin_range[-1],5))
    ...
    plt.tight_layout()
    plt.savefig(\
    "{}_SymL_last50ns_ub.pdf".format(CV), dpi=300)
    plt.show()
